...
def init_engine():
    ...
    VALUATION_ENGINE = ValuationEngine(models_dir, MASTER_DF)
...
def log_activity(user_id, activity_type, details):
    activity = Activity(user_id=user_id, activity_type=activity_type, details=details)
...
def forgot_password():
    if request.method == 'POST':
        ...
        if user:
            flash('Password reset link sent to your email (demo - not implemented)', 'success')
        else:
...
def dashboard():
    ...
    return render_template('dashboard.html', predictions=predictions, total_predictions=total_predictions, dropdown=dropdown_data)
...
def predict():
    ...
    db.session.add(pred)
    log_activity(current_user.id, 'prediction', f"Predicted {property_data['property_type']} in {property_data['city']}: INR {result['predicted_price']:,.0f}")
    db.session.commit()
...
def what_if():
    if request.method == 'GET':
        return render_template('what_if.html', original=None, modified=None, base_property={}, changes={})
    ...
    log_activity(current_user.id, 'what_if', f"What-If analysis for {base['property_type']} in {base['city']}")
    db.session.commit()
...
def comparables():
    ...
    log_activity(current_user.id, 'comparable_search', f"Comparables for {property_data['property_type']} in {property_data['city']}")
    db.session.commit()
...
def bulk_valuation():
    ...
    for col in df.columns:
        cl = col.lower().strip()
        if cl in ['property_type', 'type', 'buildtype']:
            col_map[col] = 'property_type'
        elif cl in ['city', 'location_city']:
            col_map[col] = 'city'
        elif cl in ['locality', 'location', 'area_name', 'address']:
            col_map[col] = 'locality'
        elif cl in ['area', 'area_sqft', 'size', 'total_area', 'super area']:
            col_map[col] = 'area_sqft'
        elif cl in ['bhk', 'bedrooms', 'no. of bedrooms', 'bedroom']:
            col_map[col] = 'bhk'
        elif cl in ['bathrooms', 'bath', 'bathroom']:
    ...
    for idx, row in df.iterrows():
        prop = {
            ...
            'bhk': int(row.get('bhk', 2)) if pd.notna(row.get('bhk')) else 2,
            'bathrooms': int(row.get('bathrooms', 2)) if pd.notna(row.get('bathrooms')) else 2,
            'property_age': int(row.get('property_age', 5)) if pd.notna(row.get('property_age')) else 5,
        }
        ...
        if 'error' not in pred:
            results.append({
                ...
                'recommendation': pred['fair_listing']['recommendation'],
            })

    log_activity(current_user.id, 'bulk_valuation', f"Bulk valuation of {len(results)} properties")
    db.session.commit()

    return render_template('bulk_valuation.html', results=results, count=len(results))
    ...
    if MASTER_DF is not None and len(MASTER_DF) > 0:
        for city in MASTER_DF['city'].unique():
            ...
            city_stats[city] = {
                ...
                'avg_price_per_sqft': round(city_df['price_per_sqft'].mean(), 2) if 'price_per_sqft' in city_df.columns else 0,
...
def experiments():
            ...
            for f in os.listdir(exp_dir):
                if f.endswith('_metadata.json'):
                    with open(os.path.join(exp_dir, f)) as fh:
                        meta = json.load(fh)
                    pt = meta.get('property_type', f.replace('_metadata.json', ''))
    ...
    return render_template('experiments.html', experiments=experiments_data, best_models=best_models)
...
def admin_dashboard():
    ...
    unique_viewed_users = db.session.query(Prediction.user_id).distinct().count()

    all_predictions = Prediction.query.order_by(Prediction.created_at.desc()).limit(10).all()
...
def admin_users():
    ...
    for u in users:
        ...
        user_activity[u.id] = {'predictions': pred_count, 'activities': act_count}
...
def admin_analytics():
    ...
    location_views = db.session.query(
        Prediction.city, db.func.count(Prediction.id)
    ).group_by(Prediction.city).order_by(db.func.count(Prediction.id).desc()).limit(10).all()

    locality_views = db.session.query(
        Prediction.locality, db.func.count(Prediction.id)
    ).group_by(Prediction.locality).order_by(db.func.count(Prediction.id).desc()).limit(10).all()
...
if __name__ == '__main__':
    with app.app_context():
        ...
        if not User.query.filter_by(username='admin').first():
            ...
            admin = User(username='admin', email='admin@propwise.ai', password_hash=pw, role='admin')
